...
export function AccuracyBar({ pct }: { pct: number }) {
  const color = pct >= 80 ? 'bg-blue-600' : pct >= 60 ? 'bg-blue-400' : 'bg-blue-300';
  return (
    <div className="flex items-center gap-2 min-w-[96px]">
      <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
        <div className={`h-full rounded-full ${color}`} style={{ width: `${pct}%` }} />
      </div>
      <span className="text-xs font-semibold w-9 text-right text-slate-700">{pct}%</span>
    </div>
  );
}
              ...
              return (
                <tr key={name} className={`hover:bg-gray-50 transition-colors ${empty ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-3 py-3 text-center font-semibold text-slate-700">
                    <span>{agg.correct}</span>
                    <span className="text-gray-300"> / </span>{agg.total}
                  </td>
                  <td className={`px-3 py-3 text-center ${agg.incorrect > 0 ? 'font-semibold text-slate-800' : 'text-slate-400 font-normal'}`}>{agg.incorrect}</td>
                  <td className={`px-3 py-3 text-center ${agg.doubts > 0 ? 'font-semibold text-slate-800' : 'text-slate-400 font-normal'}`}>{agg.doubts}</td>
                  <td className={`px-3 py-3 text-center ${agg.cleared > 0 ? 'font-semibold text-slate-800' : 'text-slate-400 font-normal'}`}>{agg.cleared}</td>
