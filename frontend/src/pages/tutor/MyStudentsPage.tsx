      ...
      const allStudentData = await Promise.all(
        students.map(async (student) => {
          try {
            ...
            if (analytics.sectionStats?.length > 0) {
              const engSections = analytics.sectionStats.filter((s: any) => /reading|writing|rw|english/i.test(s.sectionName));
              const mathSections = analytics.sectionStats.filter((s: any) => /math/i.test(s.sectionName));
                          ...
                          {filteredRows.map((r, i) => {
                            ...
                            return (
                              <tr key={r.id} onClick={() => setSelectedAttemptId(r.id)}
                                ...
                                <td className="px-4 py-3 text-center text-xs text-slate-500 border-r border-slate-100">{formatProgressDate(r.startedAt)}</td>
                                <td className="px-4 py-3 text-center text-xs text-slate-500 border-r border-slate-100">{formatProgressDate(r.completedAt)}</td>
                    ...
                    .map((sa: any) => {
                      const allQs = sa.section.questions.flatMap((tq: any) => {
                        ...
                        if (isPassage && q.childQuestions?.length > 0) {
                          return q.childQuestions.map((cq: any) => ({
                            ...tq,
                            questionId: cq.id,
                            question: {
                              ...cq,
