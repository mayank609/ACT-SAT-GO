...
function QuestionReviewItem({ question, index, attempt }: QuestionReviewItemProps) {
  ...
  return (
    <div className={`border-2 rounded-xl overflow-hidden ${isCorrect ? 'border-emerald-200' : isSkipped ? 'border-slate-200' : 'border-red-200'}`}>
      <div className={`px-4 py-3 flex items-start gap-3 ${isCorrect ? 'bg-emerald-50' : isSkipped ? 'bg-slate-50' : 'bg-red-50'}`}>
        <div className="flex items-center gap-2 flex-shrink-0">
          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white ${isCorrect ? 'bg-emerald-500' : isSkipped ? 'bg-slate-400' : 'bg-red-500'}`}>
            {index + 1}
          </div>
          {isCorrect ? <CheckCircle size={16} className="text-emerald-600" /> : <XCircle size={16} className={isSkipped ? 'text-slate-400' : 'text-red-500'} />}
        </div>
        <p className="text-sm text-slate-800 flex-1 leading-relaxed">
          {question.text || `Question ${index + 1}: Sample question text`}
        </p>
        <div className="flex items-center gap-1.5 flex-shrink-0">
          {attempt?.timeSpent && <span className="text-xs text-slate-500 flex items-center gap-1"><Clock size={10} />{attempt.timeSpent}s</span>}
      ...
      <div className="px-4 py-3 bg-white">
        {question.options && (
          <div className="space-y-2 mb-3">
            {question.options.map((opt) => {
              ...
              return (
                <div key={opt.id} className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm ${
                  isCorrectOption ? 'bg-emerald-50 border border-emerald-200' :
                  isUserAnswer && !isCorrectOption ? 'bg-red-50 border border-red-200' :
                  ...
                  <div className={`w-6 h-6 rounded-full border flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                    ...
                    isUserAnswer ? 'border-red-400 bg-red-400 text-white' :
                    'border-slate-300 text-slate-500'
                  }`}>
                    {opt.id.toUpperCase()}
                  </div>
                  <span className={isCorrectOption ? 'text-emerald-800 font-medium' : isUserAnswer ? 'text-red-700 line-through' : 'text-slate-600'}>
                  ...
                  {isUserAnswer && !isCorrectOption && <span className="ml-auto text-xs text-red-500 font-medium">Your answer</span>}
                </div>
              );
            })}
          </div>
        ...
        {question.type === 'numeric' && (
          <div className="flex gap-4 mb-3">
            <div className="text-sm">
              ...
              <span className={`font-bold ${isCorrect ? 'text-emerald-600' : 'text-red-600'}`}>{String(userAnswer ?? '—')}</span>
            </div>
            <div className="text-sm">
              <span className="text-slate-500">Correct: </span>
              <span className="font-bold text-emerald-600">{String(correctAnswer)}</span>
            </div>
          </div>
        )}
        ...
        {showExplanation && question.explanation && (
          <div className="mt-2 p-3 bg-blue-50 rounded-xl text-xs text-blue-900">
            {question.explanation}
          </div>
        )}
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => navigate(-1)} className="p-2 rounded-lg text-slate-500 hover:bg-slate-100">
          <ArrowLeft size={18} />
        </button>
        <div>
          <h1 className="text-xl font-bold text-slate-900">Test Review: {test.title}</h1>
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-6 text-white">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="md:col-span-1 text-center">
            <p className="text-5xl font-bold">{MOCK_ANALYTICS.totalScore}</p>
            <p className="text-blue-200 text-sm mt-1">Composite Score</p>
            <p className="text-blue-300 text-xs">(out of 36)</p>
          </div>
          {MOCK_ANALYTICS.sections.map((sec) => (
            <div key={sec.sectionId} className="text-center">
              <p className="text-3xl font-bold">{sec.accuracy.toFixed(0)}%</p>
              <p className="text-blue-200 text-sm mt-1">{sec.sectionName}</p>
              <p className="text-blue-300 text-xs">{sec.correct}/{sec.totalQuestions} correct</p>
            </div>
          ))}
        </div>
        <div className="mt-5 pt-4 border-t border-blue-500/50 grid grid-cols-3 gap-4 text-center">
          <div>
            <p className="text-xl font-bold">{MOCK_ANALYTICS.overallAccuracy}%</p>
            <p className="text-blue-300 text-xs">Overall Accuracy</p>
          </div>
          <div>
            <p className="text-xl font-bold">{MOCK_ANALYTICS.percentile}th</p>
            <p className="text-blue-300 text-xs">Percentile</p>
          </div>
          <div>
            <p className="text-xl font-bold">{Math.round(MOCK_ANALYTICS.totalTimeUsed / 60)}/{Math.round(MOCK_ANALYTICS.totalTimeAllocated / 60)}</p>
            <p className="text-blue-300 text-xs">Minutes Used</p>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 p-6">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={sectionBarData} barGap={4}>
              ...
              <Bar dataKey="correct" fill="#10b981" radius={[4, 4, 0, 0]} name="Correct" barSize={22} />
              <Bar dataKey="incorrect" fill="#ef4444" radius={[4, 4, 0, 0]} name="Incorrect" barSize={22} />
              <Bar dataKey="skipped" fill="#e2e8f0" radius={[4, 4, 0, 0]} name="Skipped" barSize={22} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      {/* Section-level tabs for Q review */}
          ...
          {test.sections.map((sec, idx) => {
            ...
            return (
              <button
                ...
                className={`flex-shrink-0 px-5 py-3.5 text-sm font-medium border-b-2 -mb-px transition-all ${
                  activeSection === idx ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700'
                }`}
              >
                {sec.name}
                {secAna && (
                  <span className={`ml-2 text-xs ${activeSection === idx ? 'text-blue-400' : 'text-slate-400'}`}>
                    {secAna.correct}/{secAna.totalQuestions}
                  </span>
                )}
              </button>
            );
          ...
          {MOCK_ANALYTICS.sections[activeSection] && (
            <div className="grid grid-cols-4 gap-3 mb-6">
              {[
                ...
                { label: 'Incorrect', value: MOCK_ANALYTICS.sections[activeSection].incorrect, color: 'text-red-600', bg: 'bg-red-50' },
      ...
      <div className="flex gap-3 justify-end">
        ...
        <Button variant="secondary" icon={<ArrowLeft size={14} />} onClick={() => navigate('/my-progress')}>
          View Progress
        </Button>
      </div>
    </div>
